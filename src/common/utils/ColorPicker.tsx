...
export function ProColorPicker({
  ...
  const [rgba, setRgba] = useState<RGBA>(() => hexToRgba(value || "#4D00D4", 1));
  ...
  return (
    <div className="w-[280px] rounded-xl border bg-white shadow p-3 space-y-3">
      {/* SV Square */}
      <div className="rounded overflow-hidden">
        <RgbaColorPicker color={rgba} onChange={setRGBA} />
      </div>

      {/* Hue bar is included by react-colorful inside the picker */}

      {/* Inputs */}
      <div className="grid grid-cols-5 gap-2 text-xs">
        <div className="col-span-2">
          <label className="text-gray-500">Hex</label>
          <HexColorInput
            prefixed
        ...
        {(["r","g","b","a"] as const).map((k) => (
          <div key={k}>
            <label className="text-gray-500 uppercase">{k}</label>
            <input
              type="number"
              min={k === "a" ? 0 : 0}
              max={k === "a" ? 1 : 255}
